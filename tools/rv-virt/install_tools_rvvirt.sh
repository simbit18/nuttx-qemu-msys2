...
riscv_gcc_toolchain() {
  ...
  if [ ! -f "${NUTTXTOOLS}/riscv-none-elf-gcc/bin/riscv-none-elf-gcc" ]; then
    ...
    curl -O -L --depth 1 https://github.com/xpack-dev-tools/riscv-none-elf-gcc-xpack/releases/download/v13.2.0-2/${basefile}.zip
    unzip -qo ${basefile}.zip
...
install_build_tools() {
  ...
  install="gen_romfs kconfig_frontends riscv_gcc_toolchain"
